...
- Add a `.travis.yml` file to the project root which defines all the configuation,
- Add environment vairables in the travis ci, which are mentioned in .travis.yml with `$` prefix
- Now upon every new commit push, the travis ci will be invoked


=== Setup New Relic

New Relic provide a gem and a licence file, just add the gem to the Gemfile, run and add the licence in the config/ folder.

- Signup for a newrelic account
- Create a new app, and download the licence yml file
- Add `newrelic` gem to Gemfile
- Run `bundle install`
- Add the licence to config/ folder
- Tweak the configuration as required in the licence file.

=== Setup Docker

All the docker relation config is in Dockerfile in the project root. To make a container, here is the steps
...
Open http://localhost:3000 in the browser to try it out.
...
== Produciton Setup using AWS

=== Setup App

- Login to AWS Account
- Go to AWS Elasticbeanstalk https://console.aws.amazon.com/elasticbeanstalk/home
- Create a new application with the following steps/configuraiton
